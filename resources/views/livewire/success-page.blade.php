      ...
      <div>
          ...
          <div class="flex items-start justify-start flex-shrink-0">
            <div class="flex items-center justify-center w-full pb-6 space-x-4 md:justify-start">
              <div class="flex flex-col items-start justify-start space-y-2">
                ...
                <p class="text-sm leading-4 text-gray-600 dark:text-gray-400">{{ $order->address->street }}</p>
                <p class="text-sm leading-4 text-gray-600 dark:text-gray-400">{{ $order->address->city }}, {{ $order->address->state }}, {{ $order->address->postal_code }}</p>
        ...
        <div class="flex flex-wrap items-center pb-4 mb-10 border-b border-gray-200 dark:border-gray-700">
          ...
          <div class="w-full px-4 mb-4 md:w-1/4">
            ...
            <p class="text-base font-semibold leading-4 text-blue-600 dark:text-gray-400">₹{{ Number::currency($order->grand_total,'INR')}}</p>
          </div>
          <div class="w-full px-4 mb-4 md:w-1/4">
            <p class="mb-2 text-sm leading-5 text-gray-600 dark:text-gray-400">Payment Method:</p>
            <p class="text-base font-semibold leading-4 text-gray-800 dark:text-gray-400">{{ $order->payment_method == 'cod'? 'Cash on Delievery':""}}</p>
          </div>
        </div>
          ...
          <div class="flex flex-col items-stretch justify-center w-full space-y-4 md:flex-row md:space-y-0 md:space-x-8">
            <div class="flex flex-col w-full space-y-6">
              ...
              <div class="flex flex-col items-center justify-center w-full pb-4 space-y-4 border-b border-gray-200 dark:border-gray-700">
                <div class="flex justify-between w-full">
                  ...
                  <p class="text-base leading-4 text-gray-600 dark:text-gray-400">₹{{ number_format($order->subtotal, 2) }}</p>
                </div>
                <div class="flex items-center justify-between w-full">
                  <p class="text-base leading-4 text-gray-800 dark:text-gray-400">Discount</p>
                  <p class="text-base leading-4 text-gray-600 dark:text-gray-400">₹{{ number_format($order->discount, 2) }}</p>
                </div>
                <div class="flex items-center justify-between w-full">
                  <p class="text-base leading-4 text-gray-800 dark:text-gray-400">Shipping</p>
                  <p class="text-base leading-4 text-gray-600 dark:text-gray-400">₹{{ number_format($order->shipping, 2) }}</p>
                </div>
              </div>
              <div class="flex items-center justify-between w-full">
                <p class="text-base font-semibold leading-4 text-gray-800 dark:text-gray-400">Total</p>
                <p class="text-base font-semibold leading-4 text-gray-600 dark:text-gray-400">₹{{ number_format($order->total, 2) }}</p>
              </div>
            </div>
            <div class="flex flex-col w-full px-2 space-y-4 md:px-8 ">
              ...
              <div class="flex items-start justify-between w-full">
                ...
                <p class="text-lg font-semibold leading-6 text-gray-800 dark:text-gray-400">₹{{ number_format($order->shipping_cost, 2) }}</p>
              </div>
            </div>
          </div>
        </div>
        <div class="flex items-center justify-start gap-4 mb-4">
          <a href="{{ url('/orders') }}" class="inline-flex items-center justify-center w-full p-4 text-sm font-medium text-center text-white bg-blue-600 rounded-md hover:bg-blue-700 focus:ring-4 focus:ring-blue-300 dark:bg-blue-500 dark:hover:bg-blue-600 dark:focus:ring-blue-800">
            View Orders
          </a>
        </div>
      </div>
